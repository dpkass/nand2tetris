...
public class JackTokenizer {
    ...
    private void tokenize(List<String> lines) {
        ...
        for (String curr : lines) {
            String[] currArr = split(curr, symbols + " ", true);

            for (int i = 0; i < currArr.length; i++) {
                String word = currArr[i];

                if (word.startsWith("\"")) {
                    String s = getStringNumber(curr, stringCount++);
                    if (!s.isBlank()) {
                        tokens.add("\"" + s + "\"");
                        i += s.chars().filter(c -> c == ' ').count();
                    }
    ...
    private String getStringNumber(String curr, int stringCount) {
        int start = 0, end = -1;
        for (int i = 0; i <= stringCount; i++) {
            start = curr.indexOf("\"", end + 1);
            end = curr.indexOf("\"", start + 1);
        }
        return start == -1 ? "" : curr.substring(start + 1, end);
    }
    ...
    public static String[] split(String line, String delim, boolean includeDelim) {
